...
```jinja2
# {{ ansible_managed }}
# GitHub App deploy key configuration
Host github.com
  HostName github.com
  User git
  IdentityFile /home/{{ service_account }}/.ssh/github-app-key
  IdentitiesOnly yes
...
- name: Install GitHub App private key
  copy:
    content: "{{ vault_github_app_key }}"
    dest: "/home/{{ service_account }}/.ssh/github-app-key"
    owner: "{{ service_account }}"
    group: "{{ service_account }}"
    mode: "0600"
...
git commit -m "feat(infra): add Ansible dsv-app role — VM identity and GitHub App key"
```
...
# GitHub App deploy key (private key)
