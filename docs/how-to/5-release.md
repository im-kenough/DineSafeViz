...
3. **Review the draft release.** Open the
   ...
   - The **Tag** field (top of the page) matches the version you intend to
     publish. If not, edit it to match (e.g., `v0.2.0`). The publish workflow
...
[Overriding the version](#overriding-the-version) below.
...
### How the draft accumulates changes
...
### Release Drafter vs GitHub's native release notes
...
### Why `release.yml` uses the GitHub API instead of `softprops/action-gh-release`
...
### Release Drafter cannot target historical commits
